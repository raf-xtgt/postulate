...
export default function SessionFlow({ onClose, sessionGuid }: SessionFlow) {
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        sessionConvFlow.forEach((item, index) => {
            initialNodes.push({
                id: item.id.toString(),
                type: 'customNode',
                position: { x: index * nodeSpacing, y: item.branch_level * branchSpacing },
                data: { 
                    label: item.label,
                    color: item.role === 'salesman' ? '#2E7D32' : '#FFC107' // Green for salesman, Yellow for client_agent
                },
            });
